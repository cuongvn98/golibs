...
// After a call to Hijack the HTTP apiserver library
...
func (w *wrapResponseWriter) Hijack() (net.Conn, *bufio.ReadWriter, error) {
	...
	if !ok {
		return nil, nil, fmt.Errorf("wrap response writer hijack failed")
	}
	return hj.Hijack()
}
...
// may be unavailable. For HTTP/2 requests, the Go HTTP apiserver permits
